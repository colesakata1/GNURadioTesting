...
while True:
    ...
    xc.set_noise_select(NOISE_ENABLE)
    xc.set_Sig_Type(COSINE_WAVE)
    P_avg = zmq_measure(socket1,PRINT_MEASUREMENTS)
    print(f"	Average Power: {P_avg:.4}")
    time.sleep(t_pause)

    print("Complex Sine, Filter Emabled: ")
    xc.set_cmplx_select(COMPLEX_SINE)
    xc.set_sig_freq(500000)
    xc.set_filter_select(FILTER_BYPASS)
    xc.set_Sig_Type(SQUARE_WAVE)
    time.sleep(t_pause)
    P_avg = zmq_measure(socket1,PRINT_MEASUREMENTS)
    print(f"	Average Power: {P_avg:.4}")
    time.sleep(t_pause)
